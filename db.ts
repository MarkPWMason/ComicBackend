...
const returnCharacters = (callback: any, errorCallback: any) => {
  /*
  ...
  */
  db.query('SELECT characters.character_id, characters.character_name, popularity.clicks FROM characters LEFT JOIN popularity ON characters.character_id = popularity.character_id ORDER BY popularity.clicks DESC', function (error: any, results: any) {
    if (error) {
      errorCallback(error);
    }
    console.log(results)
...
const incrementCharacterClick = (
  character_id: number,
  callback: any,
  errorCallback: any
) => {
  db.query(
    'SELECT * FROM popularity WHERE character_id = ?',
    character_id,
    function (error: any, results: any) {
      if (error) {
        ...
        if (results.length === 0) {
          //Insert row
          db.query(
            'INSERT INTO popularity (character_id, clicks) VALUES (?, ?)',
            [character_id, 1],
            function (insertErr: any, insertRes: any) {
          ...
          db.query(
            'UPDATE popularity SET clicks = ? WHERE character_id = ?',
            [results[0].clicks + 1, character_id],
            function (updateErr: any, updateRes: any) {
